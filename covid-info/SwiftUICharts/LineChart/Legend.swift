...
struct Legend: View {
    ...
    var body: some View {
        ZStack(alignment: .topLeading){
            ...
            Text("\(self.getXLegend()!.joined(separator:"\t"))").offset(x: 40, y: 220)
            .foregroundColor(Colors.LegendText)
            .font(.caption)
        }
    }
    ...
    func getXLegend() -> [String]? {
        ...
        if (points.count < 4) {
            guard let max : ChartDataPoint = points.max() else { return nil }
            guard let min : ChartDataPoint = points.min() else { return nil }
        ...
        var index = 0
        var itemsAtEvenIndices : [String] = []
        for item in points {
          if index % 2 == 0 {
            itemsAtEvenIndices.append(item.key)
          }
          index += 1
        }
...
struct Legend_Previews: PreviewProvider {
    static var previews: some View {
        GeometryReader{ geometry in
            Legend(data: TestData.values, frame: .constant(geometry.frame(in: .local)), hideHorizontalLines: .constant(false))
